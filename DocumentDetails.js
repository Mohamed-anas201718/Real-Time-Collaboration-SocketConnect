...
const DocumentDetails = () => {
    const socket = useRef(null); // Using useRef for a persistent socket connection
    const { id } = useParams();
    ...
    const [successMessage, setSuccessMessage] = useState(''); // Fixed state declaration
    ...
    useEffect(() => {
        ...
        socket.current.emit('joinDocument', id);
        
        // Listen for updates
        socket.current.on('receiveUpdate', (updateData) => {
            if (updateData.content) setContent(updateData.content);
        });

        socket.current.on('receiveUpdateTitle', (updateData) => {
            if (updateData.title) setTitle(updateData.title);
        });
    ...
    const handleUpdate = async () => {
        try {
            ...
            socket.current.emit('documentUpdate', { documentId: id, title, content });
            setSuccessMessage('Document updated successfully!'); // Fixed state usage
        } catch (error) {
            ...
            <div className="form-group">
                ...
                <input
                    ...
                    onChange={(e) => {
                        ...
                        socket.current.emit('documentUpdate', { documentId: id, title: e.target.value, content });
                    }}
                />
            </div>
            <div className="form-group mt-3">
                ...
                <textarea
                    ...
                    onChange={(e) => {
                        ...
                        socket.current.emit('documentUpdate', { documentId: id, title, content: e.target.value });
                    }}
                />
            </div>
            <div className="mt-3">
